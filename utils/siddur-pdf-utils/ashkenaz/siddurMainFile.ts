...
const ensureSpaceAndDraw = (
  ...
  // The yOffset for the last line is the total height used downwards, relative to the block's start.
  // We add the first line's height to ensure enough space for the block itself.
...
export const generateSiddurPDF = async ({
  ...
  // --- Font Loading ---
  const hebrewFontPath = path.join(
    process.cwd(),
    'fonts',
