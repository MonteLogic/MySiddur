...
// --- Define Paths ---
const prayersDir = path.join(__dirname, 'packages/prayer/database');

// --- MODIFIED --- Output paths are now cleaner
const outputDataDir = path.join(__dirname, 'packages/dist/data');
const outputIndexFile = path.join(__dirname, 'packages/dist/index-data.ts');
...
export const prayerIndex: { [key: string]: any } = ${JSON.stringify(
  prayerIndex,
  null,
  2
)};
